...
public removeadminfn(id, level, cid)
{
    ...
    if (!target[0]) {
        console_print(id, "[%s] Usage:  amx_removeadmin <auth|steamid|ip|name> - remove admin", PLUGINNAME);
        return PLUGIN_HANDLED;
    }
    ...
    new usersFile[192];
    new tmpFile[192];
    format(usersFile, charsmax(usersFile), "%s/users.ini", cfgDir);
    format(tmpFile, charsmax(tmpFile), "%s/users_temp.ini", cfgDir);

    if (!file_exists(usersFile)) {
        console_print(id, "[%s] File ^"%s^" doesn't exist.", PLUGINNAME, usersFile);
        return PLUGIN_HANDLED;
    }
